...
BATCH_SIZE = 100
INPUT_SHAPE = (124, 124)
CNN_INPUT = (INPUT_SHAPE[0], INPUT_SHAPE[1], 3) # If gray 3rd parameter should be 1, else if RGB go with 3
EPOCHS = 100
MODEL_NAME = "model-opt-LARGE-4070"
...
model = Sequential([
    ...
    # Block 1
    Conv2D(84, kernel_size=3, activation="relu", input_shape=CNN_INPUT),
    ...
    # Block 2
    Conv2D(160, kernel_size=3, activation="relu", kernel_regularizer=REGULARLIZER),
    Dropout(0.2),
    Conv2D(172, kernel_size=3, activation="relu", kernel_regularizer=REGULARLIZER),
    Dropout(0.3),
    Conv2D(194, kernel_size=3, activation="relu", kernel_regularizer=REGULARLIZER),
    BatchNormalization(),
    MaxPooling2D(pool_size=(2, 2)),

    # Block 3
    Conv2D(264, kernel_size=3, activation="relu", padding="same", kernel_regularizer=REGULARLIZER),
    BatchNormalization(),
    MaxPooling2D(pool_size=(2, 2)),

    # Block 4
    Conv2D(214, kernel_size=3, activation="relu", padding="same", kernel_regularizer=REGULARLIZER),
    Dropout(0.2),
    MaxPooling2D(pool_size=(2, 2)),
    ...
    # Classification head
    GlobalAveragePooling2D(),
    Dense(128, activation="relu", kernel_regularizer=REGULARLIZER),
    Dense(224, activation="relu", kernel_regularizer=REGULARLIZER),
    Dropout(0.5),
...
history = model.fit(
    ...
    class_weight=class_weights_dict, # cw
