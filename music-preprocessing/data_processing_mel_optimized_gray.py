...
'''
Script that takes mp3 files and converts them to grayscale mel-spectogram images.
'''
...
# Function to save mel-spectrogram images
def save_mel_image(y, sr, out_path):
    try:
        ...
        # scale to [0,255]
        S_img = np.clip((S_norm + 3) / 6 * 255, 0, 255).astype(np.uint8)
        img = Image.fromarray(S_img).resize(IMG_SIZE, Image.BICUBIC)
        img.save(out_path)
    except Exception as e:
        logger.error(f"Error saving mel image for {out_path}: {e}")

# Process tracks and save mel-spectrogram images
for tid in tqdm(track_ids):
    ...
    for seg in range(NUM_SEGMENTS):
        ...
        out_dir = Path("data_opt/large_mel_images") / genre_label
        out_dir.mkdir(parents=True, exist_ok=True)
        out_file = out_dir / f"{tid_str}_{seg:02d}.png"

        # Save mel-spectrogram image
        save_mel_image(y_seg, SR, str(out_file))

logger.info("Mel-spectrogram image extraction completed.")
